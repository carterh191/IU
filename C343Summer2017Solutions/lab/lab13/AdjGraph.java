...
public class AdjGraph implements Graph {
    ...
    //modified from walk(), adding function for components
    public void walk2(String method) {
        clearWalk();
        //traverse the graph 
        int comp = 0;
        int[] compSize = new int[totalNode];
        for(int i = 0; i < nodeList.size(); i ++) {
            if(ifVisited(i) == false) {
                if(i != 0) {
                    for(int j = 0; j < nodeList.size(); j ++) if(ifVisited(j) == true) compSize[comp] ++;
                    comp += 1; //when the traversal starts at a new node, a component is added
                }
        ...
        compSize[comp ++] = nodeList.size(); //add the last component
        //so far compSize[i] records the total number of nodes included in component 0, ...i (cumulative)
        //needs to be fixed
        for(int i = comp - 1; i > 0; i --) {
            compSize[i] -= compSize[i - 1];
        }
        System.out.println(method + ":");
        displayEnum();
        System.out.println("total components: " + comp);
        for(int i = 0; i < comp; i ++)
            System.out.println("comp: " + i + " contains " + compSize[i] + " nodes");
    }
    ...
    public static void main(String argv[]) {
        ...
        g.setEdge("B",  "C",  7);
        g.display();
        //g.walk2("BFS"); // either traversal approach should work fine
        g.walk2("DFS");
    }
}